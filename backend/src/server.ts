...
import { users, groups, categories, expenses, payments, calculateBalances } from "./repository.js";
...
function requireRole(req: Request, res: Response, groupId: string, minRole: GroupRole): boolean {
...
app.put("/api/groups/:id", (req: Request, res: Response) => {
  ...
  if (!requireRole(req, res, groupId, GroupRole.ADMIN)) return;
...
app.post("/api/groups/:groupId/expenses", (req: Request, res: Response) => {
  ...
  if (!requireRole(req, res, groupId, GroupRole.EDITOR)) return;
...
app.put("/api/expenses/:id", (req: Request, res: Response) => {
  ...
  if (!expense) return res.status(404).json({ error: "Expense not found" });
  if (!requireRole(req, res, expense.groupId, GroupRole.EDITOR)) return;
...
app.delete("/api/expenses/:id", (req: Request, res: Response) => {
  ...
  const expense = expenses.getById(expenseId);
  if (!expense) return res.status(404).json({ error: "Expense not found" });
  if (!requireRole(req, res, expense.groupId, GroupRole.ADMIN)) return;
...
app.get("/api/groups/:groupId/payments", (req: Request, res: Response) => {
  res.json(payments.getByGroup(param(req, "groupId")));
});

app.post("/api/groups/:groupId/payments", (req: Request, res: Response) => {
  const groupId = param(req, "groupId");
  if (!requireRole(req, res, groupId, GroupRole.EDITOR)) return;
...
app.delete("/api/payments/:id", (req: Request, res: Response) => {
  ...
  const payment = allPayments.find((s) => s.id === paymentId);
  if (!payment) return res.status(404).json({ error: "Payment not found" });
  if (!requireRole(req, res, payment.groupId, GroupRole.ADMIN)) return;
...
// TODO v2: Simplified debts endpoint
// app.get("/api/groups/:groupId/debts", ...)
